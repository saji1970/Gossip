...
export class MessageService {
  ...
  /**
   * Send a group message (alias for sendMessage with simpler parameters)
   */
  async sendGroupMessage(groupId: string, options: { messageType: 'text' | 'image' | 'file' | 'audio' | 'video'; content: string; fileUrl?: string; fileSize?: number }): Promise<Message> {
    return this.sendMessage(groupId, options.content, options.messageType, options.fileUrl, options.fileSize);
  }
  ...
  async getMessages(groupId: string, limit: number = 50, offset: number = 0): Promise<Message[]> {
    try {
      ...
      // Sort by timestamp (oldest first) and apply pagination
    ...
    try {
      ...
      for (const group of userGroups) {
        ...
        if (groupMessages.length > 0) {
          // Get the most recent message from each group
          const latestMessage = groupMessages
          ...
          if (latestMessage) {
            try {
              ...
              console.error('Failed to decrypt recent message:', error);
              recentMessages.push({
                ...latestMessage,
      ...
      // Sort all recent messages by timestamp (newest first)
      return recentMessages.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
    } catch (error) {
