...
export const ProfileScreen: React.FC = () => {
  ...
  const handleAvatarPress = () => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Remove Avatar', onPress: removeAvatar, style: 'destructive' },
      ]
    );
  };
  ...
  const updateUserAvatar = async (uri: string) => {
    try {
      if (user) {
        await authService.updateProfile({ avatar: uri });
  ...
  const renderFeaturesSection = () => (
    <Card style={styles.sectionCard}>
      ...
      <TouchableOpacity style={styles.settingItem} onPress={navigateToEphemeralChat}>
        <View style={styles.settingLeft}>
          <Icon name="zap" size={20} color="#6366F1" />
          <Text style={styles.settingLabel}>Ephemeral Chat</Text>
        </View>
